At the time of writing, the Grandstream GWN7710r (firmware 1.0.7.9) does not export its Input Voltage when querying the device's SNMP agent.  As a workaround, this php script logs into the device via curl / HTTP and scrapes the voltage value.  It then exports the voltage as per AEN2000 guidelines.
